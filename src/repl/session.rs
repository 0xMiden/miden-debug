...
impl ReplSession {
    ...
    pub fn new(config: Box<DebuggerConfig>) -> Result<Self, Report> {
        ...
        let editor = DefaultEditor::new().map_err(|e| Report::msg(format!("failed to create editor: {e}")))?;
    ...
    fn make_prompt(&self) -> String {
        ...
        // Use raw ANSI codes - simpler and works with rustyline
    ...
    fn run_until_stopped(&mut self) {
        ...
        loop {
            ...
            let (_op, is_op_boundary, proc, loc) = match self.state.executor.last.as_ref() {
                Some(last_state) => {
    ...
    fn cmd_stack(&mut self) -> Result<(), String> {
        let stack = self.state.executor.last.as_ref()
        ...
        for (i, elem) in stack.iter().enumerate() {
            ...
            println!("  {} [{}] {} (0x{:x})", marker, i, elem.as_int(), elem.as_int());
        }
        Ok(())
    }
    ...
    fn cmd_backtrace(&mut self) -> Result<(), String> {
        ...
        for (i, frame) in frames.iter().rev().enumerate() {
            let proc_name = frame.procedure("").unwrap_or_else(|| Rc::from("<unknown>"));
            let loc_str = frame.last_resolved(&self.state.source_manager)
                .map(|r| format!(" at {}", r))
                .unwrap_or_default();
